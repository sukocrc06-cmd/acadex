...
// ==========================================================================
// PDF export — capture live Tables/Charts/Diagrams visuals.
//
// appendStudyCardToDoc() previously only ever wrote Title/Summary/Key Terms/
// Key Points/Quiz Questions — tables, charts and diagrams generated by the
// AI were rendered on screen (Chart.js canvases, Mermaid SVGs) but never
// made it into the exported PDF at all. These helpers capture those live
// DOM visuals as PNG image data (when the study card's modal is open, so
// the canvases/SVGs actually exist), with graceful text-only fallbacks for
// the "Export All to PDF" bulk path where no modal/DOM exists for each card.
// ==========================================================================

async function captureCanvasAsImageData(canvasId) {
  try {
    const canvasEl = document.getElementById(canvasId);
    if (!canvasEl || typeof canvasEl.toDataURL !== 'function') return null;
    return canvasEl.toDataURL('image/png', 1.0);
  } catch (err) {
    console.warn('captureCanvasAsImageData failed for', canvasId, err);
    return null;
  }
}

async function captureMermaidSvgAsImageData(containerId) {
  try {
    ...
    const width = Math.max(1, Math.ceil(bbox.width || parseFloat(svgEl.getAttribute('width')) || 600));
    const height = Math.max(1, Math.ceil(bbox.height || parseFloat(svgEl.getAttribute('height')) || 400));
    ...
    const SCALE = 2; // extra resolution so the rasterized diagram stays crisp in the PDF
    return await new Promise((resolve) => {
      ...
      img.onload = () => {
        try {
          ...
          console.warn('captureMermaidSvgAsImageData draw failed:', err);
          finish(null);
        }
      };
      img.onerror = () => finish(null);
      img.src = svgDataUrl;
      setTimeout(() => finish(null), 4000); // safety net if the image never fires load/error
    });
  } catch (err) {
    console.warn('captureMermaidSvgAsImageData failed for', containerId, err);
    return null;
  }
}
...
function drawPdfTable(doc, unicodeReady, headers, rows, margin, y, maxWidth, safeText) {
  ...
  const measureCellLines = (text) => {
    doc.setFontSize(8.5);
    return doc.splitTextToSize(safeText(text), colWidth - cellPadding * 2);
  };
  ...
  const drawRow = (cells, isHeader) => {
    ...
    cells.forEach((_, ci) => {
      ...
      if (ci > 0) doc.line(cx, y, cx, y + rowHeight);
      const lines = cellLines[ci] || [];
      if (isHeader) doc.setTextColor(255, 255, 255);
      else doc.setTextColor(60, 66, 82);
      lines.forEach((line, li) => {
        doc.text(line, cx + cellPadding, y + cellPadding + (li + 1) * lineHeight - 1.5);
      });
    });
  ...
  if (headers && headers.length) {
    y = drawRow(headers.map(h => String(h)), true);
  }
  (rows || []).forEach(row => {
    const cells = headers && headers.length
      ? headers.map((_, i) => (row && row[i] !== undefined ? row[i] : ''))
      : (row || []);
    y = drawRow(cells.map(c => String(c)), false);
  });
...
function drawChartDataFallback(doc, unicodeReady, chartObj, margin, y, maxWidth, safeText) {
  ...
  labels.forEach((label, i) => {
    ...
    const line = safeText(`- ${label}: ${data[i] !== undefined ? data[i] : ''}`);
...
function appendStudyCardToDoc(doc, studyCard, unicodeReady, visualAssets) {
  ...
  if (Array.isArray(studyCard.diagrams) && studyCard.diagrams.length > 0) {
    ...
    studyCard.diagrams.forEach((d, idx) => {
      ...
      const imgData = visualAssets && visualAssets.diagramImages ? visualAssets.diagramImages[idx] : null;
      if (imgData) {
        try {
          const props = doc.getImageProperties(imgData);
          const imgWidth = maxWidth * 0.9;
          let imgHeight = (props.height / props.width) * imgWidth;
          if (imgHeight > 110) imgHeight = 110;
          if (y + imgHeight > 275) { doc.addPage(); y = 35; }
          doc.addImage(imgData, 'PNG', margin, y, imgWidth, imgHeight);
          y += imgHeight + 8;
        } catch (err) {
