...
(function () {
  ...
  function instructionFor(kind, language) {
    ...
    if (kind === 'table') {
      return `Create ONE clean academic TABLE from this slide only. Language: ${lang}.
Rules:
- layout_type "table"
- table with 2-4 columns, 2-5 rows, short cell text (max ~12 words per cell)
- DO NOT duplicate the full paragraph into the table; summarize into structured cells
- Keep slide title; replace body text with 1 short lead sentence OR 2 bullets max (not the same as every table cell)
- No invented statistics. No instruction leakage.`;
    }
    return `From this slide add ONE clean visual. Language: ${lang}.
If real numbers exist: chart (bar) with real labels (never Değer 1).
Else: comparison TABLE (2-4 cols, short cells).
Do not clone long paragraphs into the visual. Keep at most 1 short lead sentence in text.
layout_type chart or table. No instruction leakage.`;
  }
